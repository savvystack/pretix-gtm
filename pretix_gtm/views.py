...
class TagManagerSettingsForm(SettingsForm):
    gtm_container_id = forms.CharField(
        ...
        help_text=_(
            "It looks like GTM-XXXXXXX, where X is an uppercase letter or a digit."
        ),
        required=True,
    )
    gtm_tag_ga4 = forms.BooleanField(label=_("Google Analytics: GA4"), required=False)
    gtm_tag_ua = forms.BooleanField(
        label=_("Google Analytics: Universal Analytics"), required=False
    )
    gtm_tag_google_optimize = forms.BooleanField(
        label=_("Google Optimize"), required=False
    )
    gtm_tag_google_ads_conversion = forms.BooleanField(
        label=_("Google Ads Conversion Tracking"), required=False
    )
    gtm_tag_google_ads_remarketing = forms.BooleanField(
        label=_("Google Ads Remarketing"), required=False
    )
    gtm_floodlight_config_id = forms.CharField(
        label=_("Floodlight Advertiser ID"), required=False
    )
    gtm_custom_js_variables = forms.BooleanField(
        label=_("Custom JavaScript Variables"), required=False
    )
    gtm_preview_mode = forms.BooleanField(label=_("Preview Mode"), required=False)
